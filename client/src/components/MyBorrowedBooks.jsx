...
const MyBorrowedBooks = () => {
  ...
  const returnedBooks = userBorrowedBooks?.filter(book => { return book.returned === true });
  const nonReturnedBooks = userBorrowedBooks?.filter(book => { return book.returned === false });
  ...
  return <>
    <main className="relative flex-1 p-6 pt-6">
      ...
      <header className="flex flex-col gap-3 sm:flex-row md:items-center">
        <button className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-tl-lg sm:rounded-bl-lg text-center border-2 font-semibold py-2 w-full sm:w-72 cursor-pointer ${filter === "returned" ? "bg-black text-white border-black" : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}
          onClick={() => setFilter("returned")}
        >Returned Books</button>
        <button className={`relative rounded sm:rounded-tl-none sm:rounded-bl-none sm:rounded-tr-lg sm:rounded-br-lg text-center border-2 font-semibold py-2 w-full sm:w-72 cursor-pointer ${filter === "nonReturned" ? "bg-black text-white border-black" : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}
